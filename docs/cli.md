...
limits both the Zig and ZLS builds. There is no opt-in ZLS flag.

Stable Zig selectors choose the highest strict ZLS tag in the same major/minor cycle. Development
selectors use literal ZLS remote HEAD and require the source-declared release cycle to match Zig.
The exact managed Zig then builds and verifies ZLS. Any failure occurs before pointer publication.
...
references it. `gc` never removes final installations and retains profiles whenever references are
uncertain; the global profile is a retained reference.
